...
void Boss::Opening(Boss* boss, Player* character)
{
    ...
    for (int i = 0; i < 200 ; i++)
    {
        ...
        if(character->FoVLooking(boss->boss_rect,*boss->walls) && !hited_boss)
        {
            hited_boss = true;
            boss->boss_lives -= 1;
...
void Boss::Draw(PlayerAnimation& playerAnimation)
{
    bool drawanime = true;
    if (drawAttack1)
    {
        ...
        //Moves attack 1
        attack1.setPosition(attack1.getPosition().x + Dx * 10,attack1.getPosition().y - Dy*10 );
    ...
    if (drawAttack2)
    {
        ...
        attack2.setSize(sf::Vector2f(960,960));
