...
Player::Player(sf::RenderWindow* window)
{
    ...
    fovpic.loadFromFile("Assets/Images/Sprites/Boss/fov.png");
...
void Player::FoV()
{
    for (int i = 0; i < number_of_rects; ++i)
    {
        ...
        {
            FoVpic.setPosition(FoV_vector[FoV_vector.size()-1].getPosition());
        ...
        {
            FoVpic.setPosition(FoV_vector[0].getPosition());
    ...
    if (xDirection == xDirectionID::RIGHT)
    {
        FoVpic.setPosition(FoV_vector[0].getPosition().x, FoV_vector[0].getPosition().y - FoVpic.getSize().y/2);
    ...
    FoVpic.setSize(sf::Vector2f(792, 200));
